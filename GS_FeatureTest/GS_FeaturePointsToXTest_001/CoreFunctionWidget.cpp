...
{
    bool success = mShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/GS_TriangleTest_001.vs");
    ...
    success = mShader.addShaderFromSourceFile(QOpenGLShader::Geometry, ":/GS_TriangleTest_001.gs");
    if (!success) {
        qDebug() << "shaderProgram addShaderFromSourceFile failed!" << mShader.log();
        return (success);
    }

    success = mShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/GS_TriangleTest_001.fs");
    if (!success) {
        qDebug() << "shaderProgram addShaderFromSourceFile failed!" << mShader.log();
        return (success);
    }
